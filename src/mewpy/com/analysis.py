...
def sc_score(
    ...
    for org_id, biomass_id in community.organisms_biomass.items():
        ...
        if not use_pool:
            ...
            sols = solver.solve(
                objective, minimize=True, get_values=list(objective.keys()), pool_size=n_solutions, pool_gap=0.5
            )
...
def mip_score(
    ...
    noninteracting_medium, sol1 = minimal_medium(
        noninteracting.get_community_model(),
...
def mro_score(
    ...
    pairwise = {
        (o1, o2): individual_media[o1] & individual_media[o2] for o1, o2 in combinations(community.organisms, 2)
    }
    ...
    extras = AttrDict({"community_medium": medium, "individual_media": individual_media})
...
def exchanges(com: CommunityModel, solution: SimulationResult, metabolites: Union[str, List[str]] = None):
    """_summary_
