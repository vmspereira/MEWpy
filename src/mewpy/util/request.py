...
def process_entry(entry):
    ...
    if "synonyms" in entry["genes"][0].keys():
        synonyms_list = entry["genes"][0]["synonyms"]
        sysnames = " ".join([a["value"] for a in synonyms_list])
    else:
        sysnames = ""
    # ordered locus
    if "orderedLocusNames" in entry["genes"][0].keys():
        locus_list = entry["genes"][0]["orderedLocusNames"]
        locus = [a["value"] for a in locus_list]
    else:
        locus = []
