...
class Interaction(Variable, variable_type="interaction", register=True, constructor=True, checker=True):
    ...
    def __str__(self):
        ...
        expression_str = " + ".join(
            [
                f"{coefficient} = {expression.to_string()}"
    ...
    def regulatory_events(self, value: Dict[Union[float, int], Expression]):
        ...
        for coefficient, expression in self.regulatory_events.items():
            self.remove_regulatory_event(coefficient=coefficient, remove_orphans=True, history=False)
    ...
    def from_string(
        ...
        **kwargs,
        ...
        instance = cls(
            identifier=identifier, target=target, regulatory_events={coefficient: expression}, model=model, **kwargs
        )
    ...
    def add_target(self, target: "Target", history=True):
        ...
        if history:
            self.history.queue_command(
                undo_func=self.remove_target,
                undo_kwargs={"remove_from_model": True, "history": False},
                func=self.add_target,
                kwargs={"target": target, "history": history},
            )
    ...
    def remove_target(self, remove_from_model=True, history=True):
        ...
        if history:
            self.history.queue_command(
                undo_func=self.add_target,
                undo_kwargs={"target": self.target, "history": False},
                func=self.remove_target,
                kwargs={"remove_from_model": remove_from_model, "history": history},
            )
    ...
    def add_regulatory_event(self, coefficient: Union[float, int], expression: Expression, history=True):
        """
        ...
        if history:
            self.history.queue_command(
                undo_func=self.remove_regulatory_event,
                undo_kwargs={
                ...
                kwargs={"coefficient": coefficient, "expression": expression, "history": history},
            )

        to_add = []

        for regulator in expression.variables.values():
            regulator.update(interactions={self.id: self}, model=self.model)
    ...
    def remove_regulatory_event(self, coefficient: Union[float, int], remove_orphans=True, history=True):
        """
        ...
        if history:
            self.history.queue_command(
                undo_func=self.add_regulatory_event,
                undo_kwargs={"coefficient": coefficient, "expression": expression, "history": False},
                func=self.remove_regulatory_event,
                kwargs={"coefficient": coefficient, "remove_orphans": remove_orphans, "history": history},
            )
