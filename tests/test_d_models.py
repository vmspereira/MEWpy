...
class TestGERMModel(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.regulatory_reader = Reader(
            Engines.BooleanRegulatoryCSV, EC_CORE_REG_MODEL, sep=",", id_col=0, rule_col=2, aliases_cols=[1], header=0
        )
    ...
    def test_algebra(self):
        ...
        """
        rule = (
            "((NOT (o2(e)>0)) AND (NOT (no3(e)>0)) AND (NOT (no2(e)>0)) AND (NOT (4tmao(e)>0)) AND (NOT ("
            "dmso(e)>0)) AND (for(e)>0)) OR (b0001 == 1)"
        )

        from mewpy.germ.algebra import And, Expression, Or, Symbolic, parse_expression
        ...
        # regular evaluation
        state = {identifier: 1 if identifier == "b0001" else 0 for identifier in expr.symbols}
        ...
        # custom evaluation
        values = {"A": 100, "B": 80, "C": 90, "D": 95, "F": 93, "G": 300}
    ...
    def test_model(self):
        ...
        integrated_model = Model.from_types(
        ...
        reaction1 = Reaction(identifier="R0001", stoichiometry={metabolite1: -1}, bounds=(0.0, 999999), gpr=gpr1)

        rule = "b0002 and (b0003 or b0004)"

        reaction2 = Reaction.from_gpr_string(
            identifier="R0002", rule=rule, bounds=(0.0, 999999), stoichiometry={metabolite1: -1, metabolite2: 1}
        )
        ...
        rule = (
            "((NOT (o2(e)>0)) AND (NOT (no3(e)>0)) AND (NOT (no2(e)>0)) AND (NOT (4tmao(e)>0)) AND (NOT (dmso("
            "e)>0)) AND (for(e)>0)) "
        )
    ...
    def test_write(self):
        ...
        from mewpy.io import Engines, Writer, read_model, write_model

        model = read_model(self.regulatory_reader, self.metabolic_reader)

        # to sbml
        metabolic_writer = Writer(Engines.MetabolicSBML, io=MODELS_PATH.joinpath("e_coli_core_write.xml"), model=model)

        regulatory_writer = Writer(Engines.RegulatorySBML, io=MODELS_PATH.joinpath("e_coli_lac_write.xml"), model=model)
        ...
        # to json
        model_writer = Writer(Engines.JSON, io=MODELS_PATH.joinpath("e_coli_core_write.json"), model=model)
    ...
    def test_manipulation(self):
        ...
        # r18: n <-
        # r19: o ->
        from mewpy.germ.variables import Interaction, Metabolite, Reaction, Regulator, Variable
        ...
        i_g37 = Interaction(identifier="Interaction_g37", regulatory_events={1.0: i_g37_expression}, target=g37)

        i_g38 = Interaction(identifier="Interaction_g38", regulatory_events={1.0: i_g38_expression}, target=g38)

        n = Metabolite(identifier="N", charge=0, compartment="c", formula="C12H24O6")

        o = Metabolite(identifier="O", charge=2, compartment="c", formula="C12H24O12")
        ...
        r17 = Reaction(identifier="r17", bounds=(-1000, 1000), gpr=r17_gpr, stoichiometry={n: -1, o: 2})

        # exchanges
        r18 = Reaction(identifier="r18", bounds=(-1000, 0), stoichiometry={n: -1})

        r19 = Reaction(identifier="r19", bounds=(0, 1000), stoichiometry={o: -1})
        ...
        from mewpy.germ.algebra import Expression, Not, Symbol
